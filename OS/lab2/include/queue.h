...
#define LIST_INSERT_AFTER(listelm, elm, field) do{				\
	...
	/*Step 2: Judge whether listelm.next is NULL */				\
	/* if not, then assign listelm.next.pre to a proper value.*/		\
...
#define LIST_INSERT_TAIL(head, elm, field) do{					\
	/* Step 1: Get last element in the list named head */			\
	/* the pointer to which will be loaded to elm.pre */			\
	for(									\
		LIST_NEXT((elm), field) = LIST_FIRST((head));			\
		LIST_NEXT((elm), field) && LIST_NEXT(LIST_NEXT((elm), field), field);	\
		LIST_NEXT((elm), field) = LIST_NEXT(LIST_NEXT((elm), field), field)	\
	);									\
	/* Here a NULL in elm.next implies an empty list */			\
