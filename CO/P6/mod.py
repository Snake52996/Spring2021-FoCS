...
class CacheItem:
    def __init__(self, first_, second_, third_):
        self.first = first_
        self.second = second_
        self.third = third_
...
def kickoff():
    ...
    if(d.third):
        ...
        write_memory(round_block(addr), d.second)
...
def read(addr):
    ...
    if index not in cache.keys():
        ...
        cache[index] = CacheItem(cache_size, block, False)
        cache_size = cache_size + 1
    offset = addr % block_size
    x = cache[index].second[offset]
    t = cache[index].first
    for (key, value) in cache.items():
        if(value.first > t):
            value.first = value.first - 1
    cache[index].first = cache_size - 1
    return x
...
def write(addr, data):
    ...
    if index not in cache.keys():
        if cache_size >= nums:
            kickoff()
        block = read_memory(round_block(addr))
        cache[index] = CacheItem(cache_size, block, True)
        cache_size = cache_size + 1
    offset = addr % block_size
    cache[index].second[offset] = data
    t = cache[index].first
    for (key, value) in cache.items():
        if(value.first > t):
            value.first = value.first - 1
    cache[index].first = cache_size - 1
    cache[index].third = True
...
def back_to_memory():
    ...
    for index in cache.keys():
        if(cache[index].third):
            addr = index * block_size
            write_memory(addr, cache[index].second)
